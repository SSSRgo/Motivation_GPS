...
DataName='HighDimData_threshold03_Nosepoke_Rcount_1.mat';



load('HighDimData_PR_np.mat')
load('HighDimData_PR_np_200.mat')
load('HighDimData_PR_np_2_8_2_2_400.mat')
% HighDimData=[HighDimData1;HighDimData2;HighDimData];

WindowWidth=length(Timelag_per_bin);
state_number=length(HighDimData)/WindowWidth;

cmap1=colormap(othercolor('Accent4',WindowWidth));
cmap1 = [cmap1; 0 0 0];
cmap2=colormap(othercolor('Blues8',WindowWidth));
cmap2 = [cmap2; 0 0 0];
cmap3=colormap(othercolor('BrBG4',WindowWidth));
cmap3 = [cmap3; 0 0 0];
cmap4=colormap(othercolor('Dark23',WindowWidth));
cmap4 = [cmap1; 0 0 0];
cmap5=colormap(othercolor('GnBu4',WindowWidth));
cmap5 = [cmap2; 0 0 0];
cmap6=colormap(othercolor('Oranges3',WindowWidth));
cmap6 = [cmap3; 0 0 0];
cmap7=colormap(othercolor('PuBu6',WindowWidth));
cmap7 = [cmap1; 0 0 0];
cmap8=colormap(othercolor('PiYG3',WindowWidth));
cmap8 = [cmap2; 0 0 0];
cmap9=colormap(othercolor('YlOrBr7',WindowWidth));
cmap9 = [cmap3; 0 0 0];

state_color_label={cmap1,cmap2,cmap3,cmap4,cmap5,cmap6,cmap7,cmap8,cmap9}

cmap_state=colormap(othercolor('YlOrBr7',state_number));
cmap_state = [cmap_state; 0 0 0];
cmap_time=colormap(othercolor('YlGn8',WindowWidth));
cmap_time = [cmap_time; 0 0 0];

color_index = round(1 + ((Timelag_per_bin - min(Timelag_per_bin)) / (max(Timelag_per_bin) - min(Timelag_per_bin))) * (WindowWidth - 1));


Whole_Timelag_per_bin= repmat(Timelag_per_bin, state_number, 1);

[pathstr, session, ext] = fileparts(DataName);
% load(DataName)
constantsBP
% Timelag_per_bin(201:end)=Timelag_per_bin(201:end)+7;
[a,b]=find(isnan(HighDimData));
HighDimData(:, unique(b)) = [];
...
number_of_active_neurons = nansum(filtered_full_neuron_firing, 2);
...
for state=1:state_number
    cmap=state_color_label{state};
scatter3(reduced_data((state-1)*WindowWidth+1:state*WindowWidth,2),reduced_data((state-1)*WindowWidth+1:state*WindowWidth,3),reduced_data((state-1)*WindowWidth+1:state*WindowWidth,4), 20, cmap_state(state, :), 'fill');
hold on
end
legend
...
for state=1:state_number
    cmap=state_color_label{state};
scatter3(reduced_data((state-1)*WindowWidth+1:state*WindowWidth,2),reduced_data((state-1)*WindowWidth+1:state*WindowWidth,3),reduced_data((state-1)*WindowWidth+1:state*WindowWidth,4), 20, cmap_time(color_index, :), 'fill');
hold on
end
legend

% scatter3(reduced_data(1:WindowWidth, 2), reduced_data(1:WindowWidth, 3), reduced_data(1:WindowWidth, 4), 20, cmap21(color_index, :), 'fill');
% hold on 
% scatter3(reduced_data(WindowWidth+1:2*WindowWidth, 2), reduced_data(WindowWidth+1:2*WindowWidth, 3), reduced_data(WindowWidth+1:2*WindowWidth, 4), 20, cmap22(color_index, :), 'fill');
% scatter3(reduced_data(2*WindowWidth+1:3*WindowWidth, 2), reduced_data(2*WindowWidth+1:3*WindowWidth, 3), reduced_data(2*WindowWidth+1:3*WindowWidth, 4), 20, cmap23(color_index, :), 'fill');
...
HistctCend=histcounts(idx,1:WindowWidth);
...
f2=figure;
scatter(reduced_data(:, 2), reduced_data(:, 3), 5, Whole_Timelag_per_bin, 'fill');
axis equal;
...
for state=1:state_number
    cmap=state_color_label{state};
scatter3(x((state-1)*WindowWidth+1:state*WindowWidth,:)*V(:,1),x((state-1)*WindowWidth+1:state*WindowWidth,:)*V(:,2),x((state-1)*WindowWidth+1:state*WindowWidth,:)*V(:,3), 20, cmap_state(state, :), 'fill');
hold on
end
legend
xlabel('Comp. 1');
...
legend('Whole session','Opto','NoOptp');
xlabel('ln(fr) of Half1');
ylabel('Dfr Half1-Half2');
title('Shuffle-WAKE');

f33=figure;
for state=1:state_number
    cmap=state_color_label{state};
scatter3(x((state-1)*WindowWidth+1:state*WindowWidth,:)*V(:,1),x((state-1)*WindowWidth+1:state*WindowWidth,:)*V(:,2),x((state-1)*WindowWidth+1:state*WindowWidth,:)*V(:,3), 20, cmap_time(color_index, :), 'fill');
hold on
end
% legend
% colormap(cmap9)
% % colormap(othercolor('Set27'));
% scatter3(x(WindowWidth+1:2*WindowWidth,:)*V(:,1),x(WindowWidth+1:2*WindowWidth,:)*V(:,2),x(WindowWidth+1:2*WindowWidth,:)*V(:,3), 20, cmap2(color_index, :), 'fill');
% 
% % colormap(othercolor('YlOrBr7'));
% scatter3(x(2*WindowWidth+1:3*WindowWidth,:)*V(:,1),x(2*WindowWidth+1:3*WindowWidth,:)*V(:,2),x(2*WindowWidth+1:3*WindowWidth,:)*V(:,3), 20, cmap3(color_index, :), 'fill');

% scatter3(x*V(:,1),x*V(:,2),x*V(:,3), 20, Timelag_per_bin, 'fill');
% scatter3(x*V(:,1),x*V(:,2),x*V(:,3), 20, Timelag_per_bin, 'fill');

f4=figure
scatter(x*V(:,1),x*V(:,2), 5, Whole_Timelag_per_bin, 'fill');
...
scatter3(score(:,1),score(:,2),score(:,3), 20, Whole_Timelag_per_bin, 'fill');

for state=1:state_number
    cmap=state_color_label{state};
scatter3(score((state-1)*WindowWidth+1:state*WindowWidth,1),score((state-1)*WindowWidth+1:state*WindowWidth,2),score((state-1)*WindowWidth+1:state*WindowWidth,3), 20, cmap_state(state, :), 'fill');
hold on
end
legend
...
for state=1:state_number
    cmap=state_color_label{state};
scatter3(score((state-1)*WindowWidth+1:state*WindowWidth,1),score((state-1)*WindowWidth+1:state*WindowWidth,2),score((state-1)*WindowWidth+1:state*WindowWidth,3), 20, cmap_time(color_index, :), 'fill');
hold on
end
legend
...
f6=figure
scatter(score(:,1),score(:,2), 5, Whole_Timelag_per_bin, 'fill');
