...
load('HighDimData_threshold03_Nosepoke_Rcount_1.mat')
HighDimData1=HighDimData;
load('HighDimData_threshold03_Nosepoke_Rcount_2.mat')
HighDimData2=HighDimData;
load('HighDimData_threshold03_Nosepoke_Rcount_3.mat')
HighDimData3=HighDimData;

load('HighDimData_threshold03_Trayon_nonpump.mat')
HighDimData4=HighDimData;
load('HighDimData_threshold03_Trayon_pump.mat')
HighDimData5=HighDimData;

HighDimData=[HighDimData1;HighDimData2;HighDimData3;HighDimData4;HighDimData5];
...
l=length(Timelag_per_bin);
color_index_nosepk=[color_index;color_index;color_index];
color_index_trayon=[color_index;color_index];
...
scatter3(reduced_data(1:3*l, 2), reduced_data(1:3*l, 3), reduced_data(1:3*l, 4), 20, cmap21(color_index_nosepk, :), 'fill');
hold on 
scatter3(reduced_data(3*l+1:end, 2), reduced_data(3*l+1:end, 3), reduced_data(3*l+1:end, 4), 20, cmap23(color_index_trayon, :), 'fill');
% scatter3(reduced_data(2*l+1:3*l, 2), reduced_data(2*l+1:3*l, 3), reduced_data(2*l+1:3*l, 4), 20, cmap23(color_index, :), 'fill');
colormap(othercolor('PuBu6',200))
% colorm0ap(cmap2)
...
scatter3(x(1:3*l,:)*V(:,1),x(1:3*l,:)*V(:,2),x(1:3*l,:)*V(:,3), 20, cmap21(color_index_nosepk, :), 'fill');


hold on
% colormap(othercolor('Set27'));
scatter3(x(3*l+1:end,:)*V(:,1),x(3*l+1:end,:)*V(:,2),x(3*l+1:end,:)*V(:,3), 20, cmap23(color_index_trayon, :), 'fill');
colormap(othercolor('PuBu6',200))
